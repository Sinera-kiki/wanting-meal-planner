...
<p align="center">
  <img src="docs/screenshots/onboarding.png" width="320" alt="动态餐次、快捷方案与自定义偏好" />
</p>
<p align="center"><sub>动态餐次 + 快捷方案 + 按需展开的自定义偏好</sub></p>

<table>
  <tr>
    <td width="33%"><img src="docs/screenshots/weekly-plan.png" alt="周餐单" /></td>
    <td width="33%"><img src="docs/screenshots/meal-detail.png" alt="单餐步骤" /></td>
    <td width="33%"><img src="docs/screenshots/shopping-list.png" alt="采购清单" /></td>
  </tr>
  <tr>
...
| 双层个性化 | 新用户使用快捷方案，老用户可一键恢复上次设置 |
| 通用菜谱池 | 默认轮换米饭、面食、粉类和杂粮轻食，不绑定单一主食 |
| 时间与厨具 | 支持10/15/30/45分钟，并约束灶台、微波炉、电饭锅或空气炸锅 |
| 快手餐步骤 | 每顿包含时间、食材用量、营养说明和 3 步做法 |
...
为了同时降低首次使用门槛、保留长期个性化，产品采用渐进式设置：

- **通用默认**：“不挑，合理搭配”，不预设口味或主食，系统主动轮换食材；
- **快捷方案**：“15分钟快手”“家常均衡”“轻食少油”，一键完成常用设置；
- **自定义偏好**：按需展开主食、餐食风格、时间、厨具与口味；
- **偏好记忆**：生成后保存当前偏好，下次可直接“沿用上次设置”；跨周时收起旧餐单但保留偏好。

在算法层面，忌口、时间和厨具属于硬约束；口味、主食与餐食风格只是排序权重，不会被理解为每顿强制。
...
- 绿叶菜主要在前三天消耗；
- 同一主要口味按餐次数量动态限制；
- 5顿以上时限制单一主食和蛋白质的占比，保持周级多样性；
- 步骤只能使用用户选择的厨具，并支持多厨具组合；
- 每餐必须覆盖主食、蛋白质来源与蔬果；
- 炖、焖饭、卤等慢菜不能伪装成10或15分钟；
...
- `.env`、数据库配置与构建产物默认被忽略；
- GitHub Pages 仅部署静态演示模式，不暴露 LLM 调用能力；演示版会按输入动态计算库存抵扣与换菜差异；
...
- 用 LLM 处理搭配创意，用规则处理安全、一致性和周级多样性；
